...
SELECT 'Média' AS department_name, ROUND(AVG(func.salary), 0) AS media_salarial
FROM employees func
UNION
SELECT department_name, media_salarial
FROM department_media_salarial
ORDER BY media_salarial ASC;
...
FROM (
  ...
  FROM employees func
  JOIN departments dep ON func.department_id = dep.department_id
  GROUP BY dep.department_name
...
FROM 
  employees func 
  LEFT JOIN departments dep ON func.department_id = dep.department_id
GROUP BY 
  dep.department_name
